            ...
            try:
                ...
                if action == "update_location":
                    ...
                    if instance_id and instance_id in active_instances:
                        ...
                        loc1 = player_locations.get(p1)
                        loc2 = player_locations.get(p2)
                        
                        # Kiểm tra an toàn dữ liệu vị trí tránh crash do rớt mạng tạm thời
                        if not loc1 or not loc2:
                            await manager.send_personal_message({
                                "event": "error", 
                                "message": "Chưa lấy được định vị chính xác của cả hai, vui lòng đợi vài giây và thử lại!"
                            }, user_id)
                            continue
                        ...
                        if dist <= 10.0:  # Dung sai 10m an toàn cho GPS drift
                            # Thành công!
                            success_event = {"event": "quest_success", "message": "Hoàn thành xuất sắc! Mỗi người nhận được +50 EXP và +20 Xu!"}
                            await manager.send_personal_message(success_event, p1)
                            await manager.send_personal_message(success_event, p2)
                            
                            # Cập nhật EXP và xu cho cả 2 người chơi
                            try:
                                with Session(engine) as db:
                                    for uid in [p1, p2]:
                                        try:
                                            ...
                                            continue
                                        profile = db.exec(select(UserProfiles).where(UserProfiles.user_id == user_uuid)).first()
                                        if profile:
                                            profile.total_points = (profile.total_points or 0) + 50
                                            profile.points_balance = (profile.points_balance or 0) + 20
                                            profile.updated_at = datetime.utcnow()
                                            db.add(profile)
                                    db.commit()
                            except Exception as e:
                                print(f"Error adding EXP in social quest: {e}")
                            ...
                            # Đồng bộ hiển thị yêu cầu khoảng cách 10m
                            await manager.send_personal_message({
                                "event": "error", 
                                "message": f"Các bạn đang cách nhau {dist:.0f}m. Hãy tiến lại gần nhau hơn (dưới 10m)!"
                            }, user_id)
